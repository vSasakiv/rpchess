...
layout(binding = 1, set = 1) uniform SolidUBO {
    vec3 colorA;
    vec3 colorB;
    float roughness;
    float metallic;
...
void main() {
    ...
    vec3 albedo     = material.colorA;
    float metallic  = material.metallic;
    float roughness = material.roughness;
    float ao        = 1.0;

    if (material.checkerBoard == 1) {
        vec2 uv = fragPos.xz * material.uvScale;
        float checker = mod(floor(uv.x) + floor(uv.y), 2.0);
        albedo = mix(material.colorA, material.colorB, checker);
    }
